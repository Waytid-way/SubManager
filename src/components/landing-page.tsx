...
const faqs = [
  {
    question: "ต้องเชื่อมบัตรเครดิตไหม?",
    answer: "ไม่จำเป็น เริ่มจากใส่รายการที่ใช้อยู่ก่อน แล้วค่อยต่อยอดด้วยการแจ้งเตือนอัตโนมัติ",
  },
  {
    question: "แจ้งเตือนผ่านช่องทางไหน?",
    answer: "รองรับ Email และ LINE โดยตั้งเวลาเตือนก่อนตัดบิลได้ 3-5 วัน",
  },
  {
    question: "ช่วยประหยัดได้ยังไง?",
    answer: "ระบบจะรวมยอดรายเดือน + แนะนำแพ็กที่คุ้มกว่า พร้อมลิงก์สมัครแบบ affiliate",
  },
];
      ...
      <main>
        <section className="mx-auto grid max-w-6xl gap-10 px-6 py-16 md:grid-cols-2 md:items-center">
          <div className="space-y-6">
            <p className="glass-chip glass-light inline-flex text-xs font-semibold text-blue-800">
            ...
            <div className="flex flex-wrap gap-3">
              {onboardingVariant === "A" ? (
                <button className="glass-cta glass-dark">เพิ่มรายการของคุณ</button>
              ) : (
                <button className="glass-cta glass-dark">เริ่มใช้งานผ่าน LINE</button>
              )}
              <button className="glass-cta glass-light text-slate-700">ดูเดโมแดชบอร์ด</button>
            </div>
            <div className="glass-panel glass-light flex flex-wrap gap-2 p-3 text-xs font-medium text-slate-600">
              <span className="glass-chip glass-light">50K+ users tracked</span>
              <span className="glass-chip glass-light">4.8/5 satisfaction</span>
              <span className="glass-chip glass-light">99.9% alert uptime</span>
            </div>
            <p className="text-xs font-medium text-slate-500">
              Active Experiment: Onboarding Variant {onboardingVariant}
            </p>
          </div>
          <div className="glass-panel glass-light rounded-3xl p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
              <div className="glass-panel glass-light rounded-xl p-3">
            ...
            <div className="mt-8 grid gap-8 md:grid-cols-2">
              <div className="space-y-3">
                {subscriptions.map((item) => (
                  <label key={item.name} className="glass-panel glass-dark flex items-center justify-between rounded-xl p-4">
                    <span>{item.name}</span>
              ...
              <div className="glass-panel glass-dark rounded-2xl p-6">
          ...
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-3">
              {[
                ...
                <article key={title} className="glass-panel glass-light rounded-2xl p-5">
                  <h3 className="text-xl font-semibold">{title}</h3>
                  <p className="mt-2 text-slate-600">{desc}</p>
                </article>
              ))}
            </div>
            <div className="glass-panel glass-light mt-8 rounded-2xl p-6">
              <p className="text-sm text-slate-500">Dashboard Mockup</p>
              <div className="mt-3 grid gap-4 md:grid-cols-[1fr_2fr]">
                <div className="glass-panel glass-light rounded-xl p-4">
                ...
                <div className="grid gap-2 text-sm">
                  {[
                    ...
                    <div key={item} className="glass-panel glass-light rounded-lg p-3">
                      {item}
                    </div>
                  ))}
                </div>
        ...
        <section id="packs" className="py-16">
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {packEntryVariant === "A" ? (
                <button className="glass-cta glass-dark">ดูแพ็กทั้งหมด</button>
              ) : (
                <button className="glass-cta glass-dark">ทำควิซค้นหา Pack ที่ใช่</button>
              )}
              <button className="glass-chip glass-light">Student</button>
              <button className="glass-chip glass-light">Creator</button>
              <button className="glass-chip glass-light">Developer</button>
            </div>
            <div className="glass-panel glass-light mt-8 overflow-hidden rounded-2xl">
              <table className="w-full bg-transparent text-left text-sm">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={pack} className="border-t border-slate-100/80">
                      ...
                      <td className="px-4 py-3">
                        <button className="glass-cta glass-light text-slate-900">ไปหน้าข้อเสนอ</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <section id="faq" className="bg-slate-100 py-16">
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              {faqs.map((item) => (
                <article key={item.question} className="glass-panel glass-light rounded-2xl p-5">
        ...
        <section className="bg-blue-600 py-16 text-white">
          <div className="mx-auto flex max-w-6xl flex-col items-start gap-4 px-6 md:flex-row md:items-center md:justify-between">
            ...
            <button className="glass-cta glass-light bg-white/90 text-blue-700">สร้างบัญชีฟรี</button>
          </div>
        </section>
      </main>
